import { Component, OnInit } from '@angular/core'; import { Produto } from 'src/app/models/Produto.model';
import { ProdutoService } from 'src/app/produto.service'; import { ActivatedRoute, Router } from '@angular/router'; import { map, tap } from 'rxjs';
...
export class AtualizaProdutoComponent implements OnInit {
  ...
  listarProduto(): void {
    ...
    .subscribe((res: any) => {
      console.log(res[0].produto); this.produto = new
  ...
  atualizar(id: number) {
    this._produtoService.atualizarProduto(id, this.produto).subscribe(
      produto => { this.produto = new Produto(0, "", "", "", 0) }, err => { console.log("erro ao atualizar") }
    );
